...
//This function will calculate the number of electrons in the molecule
int Molecule::electron_count()
{
    int electron = 0;
    for(int i=0; i<natom; i++) {
       electron += (int) zvals[i]; //(int) is typecasting where it transforms my double zval into an int
    }
    return electron;
}
